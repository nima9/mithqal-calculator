...
<script lang="ts">
  ...
  let modeLabel = $derived(MODE_LABELS[mode]);
  ...
  const THEME_NAMES = {
    light: LIGHT_THEME,
    dark: DARK_THEME,
  } satisfies Record<Exclude<ThemeMode, "system">, string>;

  /** Apply the selected theme to the document */
  function applyTheme(themeMode: ThemeMode) {
    const theme = themeMode === "system" ? getSystemTheme() : THEME_NAMES[themeMode];
